...
export async function initializeDatabase() {
  try {
    ...
    if (existingWeeklyPlans.length === 0) {
      ...
      // Create sample tasks for the first week only if no tasks exist for it
      const firstWeekPlan = weeklyPlans[0];
      const existingTasks = await prisma.task.findMany({
        where: { weeklyPlanId: firstWeekPlan.id },
      });
      ...
      if (existingTasks.length === 0) {
        const sampleTasks = [
          ...
          },
          {
            ...
            taskType: 'WEEK_SPECIFIC' as const,
            weeklyPlanId: firstWeekPlan.id,
          },
        ];
        ...
        console.log('Sample tasks already exist for week 1, skipping creation');
      }
    }
